        ...
        try:
            ...
            try:
                ...
                if hasattr(os, 'add_dll_directory'):
                    ad = os.add_dll_directory(dll_dir)
                    print("[DLL Diagnostic] os.add_dll_directory() invoked")
                    try:
                        handle = ctypes.CDLL(dll_abs)
                        print("[DLL Diagnostic] ctypes.CDLL succeeded after add_dll_directory")
                    except Exception as e3:
                        print(f"[DLL Diagnostic] still failed after add_dll_directory: {e3}")
                    finally:
                        try:
                            ad.close()
                        except Exception:
                ...
                print(f"[DLL Diagnostic] add_dll_directory attempt raised: {e_add}")

            # Try loading via WinDLL as an alternate test and also try LoadLibraryExW for a Win32 error code
            try:
                ...
                try:
                    ...
                    h = kernel32.LoadLibraryExW(dll_abs, None, 0x00000008)
                    if not h:
                        err = kernel32.GetLastError()
                        print(f"[DLL Diagnostic] LoadLibraryExW returned NULL, GetLastError={err}")
                    else:
                        print(f"[DLL Diagnostic] LoadLibraryExW succeeded, handle={h}")
                except Exception as e3:
                    print(f"[DLL Diagnostic] LoadLibraryExW attempt failed: {e3}")
            except Exception:
                pass
        except Exception:
            pass
...
def send_through_circuit(circuit: Circuit, message: str, destination: str, directory_url: Optional[str] = None) -> Optional[str]:
    ...
    # Accept either a base URL or a /relays URL; normalize to base (no /relays)
    ...
    try:
        if _find_default_dll():
            try:
                ...
                print(f"[Core Error] C++ circuit build failed (directory={directory_base_url}): {e}")
                raise
