...
def _build_circuit_cpp(num_relays: int, message: str, destination: str, directory_url: str) -> Circuit:
    ...
    lib.matryoshka_free_cstring.argtypes = [ctypes.c_char_p]
    lib.matryoshka_free_cstring.restype = None
    ...
    try:
        ...
        lib.matryoshka_free_cstring(out_json)

    # Parse the JSON response
    entry_ip = data.get("entry_ip")
    entry_port = data.get("entry_port")
    ...
    if not all([entry_ip, entry_port, payload_b64]):
        raise RuntimeError("Invalid circuit data from C++")
